...
if [ ! -d "$REPO_DIR" ]; then
    ...
    echo "Clónalo primero con: git clone TU_REPOSITORIO_URL $REPO_DIR"
    exit 1
fi
...
# Actualizar el archivo kustomization.yaml para usar la versión específica
if [ "$VERSION" == "1" ]; then
    echo "Configurando para usar la versión 1..."
    # Modificar el archivo para usar la imagen v1
    sed -i 's|image: webapp:v[0-9]|image: webapp:v1|g' kubernetes/base/deployment.yaml
    VERSION_MSG="Despliegue de versión 1"
elif [ "$VERSION" == "2" ]; then
    echo "Configurando para usar la versión 2..."
    # Modificar el archivo para usar la imagen v2
    sed -i 's|image: webapp:v[0-9]|image: webapp:v2|g' kubernetes/base/deployment.yaml
